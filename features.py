...
class FeatureEngine:
    def __init__(self, pair: str, shared_state: Dict[str, Any]):
        ...
        self.vol_regime_history = deque(maxlen=3600)
    ...
    async def tick(self, event_time_ms: int, local_time_ms: int, skew_ms: int):
        ...
        if sec > self.current_sec:
            ...
            self.current_sec = sec  # Update BEFORE first await — race condition fix
            ...
            try:
                ...
                logging.getLogger(f"features.{self.pair}").error(
                    f"compute_bar error di bar {bar_sec}: {e}", exc_info=True
                )
                row = None

            self.trade_buffer.clear()  # Clear BEFORE first await
